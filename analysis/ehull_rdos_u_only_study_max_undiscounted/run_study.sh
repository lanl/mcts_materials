#!/bin/bash
# Max-undiscounted study: max aggregation with rollout_discount=1.0 (no depth decay)
#   gamma = 1/(max raw r_DOS across the 108 U-only compounds) = 1/2516.1664410449775
#         = 0.00039742998860786596 (normalized gamma, same as _final and _normalized studies)
#   rollout_aggregation = max (optimistic, takes the highest of n_rollout samples)
#   rollout_discount    = 1.0 (no 0.9^depth decay on extra rollout samples — all
#           samples compared on equal footing before taking the max)
# Contrasts with ehull_rdos_u_only_study_final/ (same gamma, mean aggregation)
# and ehull_rdos_u_only_study_normalized/ (same gamma, max+0.9^depth discount).
#
...
echo "=================================================="
echo "EHULL + RDOS STUDY (U-only, max undiscounted)"
