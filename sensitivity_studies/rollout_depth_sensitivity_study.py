#!/usr/bin/env python3
"""
Rollout Depth Sensitivity Study for MCTS Crystal Structure Optimization

This script performs a sensitivity analysis of the rollout depth parameter in the MCTS
simulation phase, running multiple MCTS searches with different rollout depths and
...
def run_mcts_with_rollout_depth(
    atoms, 
    energy_calculator, 
    rollout_depth: int,
    ...
    """
    Run MCTS with a specific rollout depth.
    ...
    """
    print(f"  Running with rollout depth = {rollout_depth}")
...
def create_sensitivity_plot(
    ...
    save_path: Path,
    title: str = "Rollout Depth Sensitivity Analysis"
):
    """
    Create a multi-curve plot showing best reward vs discovery order for different rollout depths
    with error bars showing variance across repeats.
    
    Args:
        results_list: List of results dictionaries from different rollout depths
        save_path: Path to save the plot
        title: Plot title
    """
    plt.figure(figsize=(14, 8))

    # Group results by rollout depth
    grouped_results = {}
    for result in results_list:
        depth = result['rollout_depth']
        if depth not in grouped_results:
            grouped_results[depth] = []
        grouped_results[depth].append(result)

    # Define colors for different curves
    rollout_depths = sorted(grouped_results.keys())
    colors = plt.cm.viridis(np.linspace(0, 1, len(rollout_depths)))

    # Track highest mean value across all curves for ylim setting
    highest_mean_value = -float('inf')
    
    for i, depth in enumerate(rollout_depths):
        repeats = grouped_results[depth]
    ...
    plt.legend(title='Rollout Depth', bbox_to_anchor=(1.05, 1), loc='upper left')
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Run rollout depth sensitivity study for MCTS crystal optimization'
    )
    ...
    parser.add_argument('--output', '-o', type=str, default='rollout_depth_sensitivity_results',
                       help='Output directory name (default: rollout_depth_sensitivity_results)')
    ...
    parser.add_argument('--n-rollout', type=int, default=5,
                       help='Number of rollout simulations per expansion (default: 5)')
    parser.add_argument('--rollout-depths', type=str, default='1,2,3,4,5',
                       help='Comma-separated rollout depths to test (default: 1,2,3,4,5)')
    parser.add_argument('--repeats', '-r', type=int, default=3,
                       help='Number of repeated runs per rollout depth (default: 3)')
    
    args = parser.parse_args()
    
    print("=" * 80)
    print("ROLLOUT DEPTH SENSITIVITY STUDY")
    print("=" * 80)
    
    # Parse rollout depths
    try:
        rollout_depths = [int(x.strip()) for x in args.rollout_depths.split(',')]
        rollout_depths = sorted(rollout_depths)
    except ValueError:
        print("❌ Error: Invalid rollout depths format. Use comma-separated integers.")
        return 1
    ...
    print(f"   Rollout depths: {rollout_depths}")
    print(f"   Repeats per depth: {args.repeats}")
    print(f"   Total runs: {len(rollout_depths) * args.repeats}")
    ...
    print(f"   This will run {len(rollout_depths) * args.repeats} separate MCTS searches...")
    
    results_list = []
    
    for i, rollout_depth in enumerate(rollout_depths, 1):
        print(f"\n   [{i}/{len(rollout_depths)}] Testing rollout depth = {rollout_depth}")
        ...
        for repeat in range(args.repeats):
            ...
            try:
                results = run_mcts_with_rollout_depth(
                    atoms=atoms,
        ...
        if repeat_results:
            # Store all repeats for this rollout depth
    ...
    sensitivity_plot_path = output_dir / "rollout_depth_sensitivity_curves.png"
    create_sensitivity_plot(
        results_list, 
        sensitivity_plot_path,
        title=f"Rollout Depth Sensitivity Analysis\\n{atoms.get_chemical_formula()} - {args.iterations} iterations"
    )
    ...
    print(f"\\n📊 ROLLOUT DEPTH PERFORMANCE SUMMARY:")
    print("-" * 80)
    print(f"{'Depth':>6} | {'Mean Reward':>12} | {'Std Reward':>11} | {'Mean Compounds':>14} | {'Efficiency':>10}")
    print("-" * 80)
    
    # Group results by rollout depth and calculate statistics
    grouped_results = {}
    for result in results_list:
        depth = result['rollout_depth']
        if depth not in grouped_results:
    ...
    for depth in sorted(grouped_results.keys()):
        repeats = grouped_results[depth]
        ...
        print(f"{depth:>6} | {mean_reward:8.4f} ± {std_reward:4.3f} | {mean_compounds:8.1f} | {efficiency:8.4f}")
    ...
    print(f"📈 Key files:")
    print(f"   • rollout_depth_sensitivity_curves.png - Multi-curve progress plot")
    print(f"   • sensitivity_results.json - Raw numerical results")
    
    print(f"\\n💡 Recommended usage:")
    print(f"   python run_mcts.py --rollout-depth {best_depth}")
